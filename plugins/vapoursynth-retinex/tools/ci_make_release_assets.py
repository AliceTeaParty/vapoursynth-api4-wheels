#!/usr/bin/env python3
"""Create release-ready Retinex payload and wheel assets."""
...
PLUGIN_NAME = "retinex"
DEFAULT_ZIP_NAME = "retinex-msys2-ucrt64.zip"
...
def create_package_zip(package_dir: Path, out_path: Path) -> None:
    ...
    required = [package_dir / "manifest.vs", *package_dir.glob(f"{PLUGIN_NAME}.*")]
    if len(required) < 2 or not all(path.is_file() for path in required):
        raise FileNotFoundError(f"missing manifest or native plugin in {package_dir}")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Create release-ready Retinex zip and wheel assets.")
    parser.add_argument("--package-dir", default="dist/msys2-ucrt64/retinex")
