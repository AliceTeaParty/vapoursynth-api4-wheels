...
def write_include_shim(vs_pkg: Path) -> None:
    include_dir = vs_pkg / "include"
    nested = include_dir / "vapoursynth"
    nested.mkdir(parents=True, exist_ok=True)
    for header in ["VapourSynth4.h", "VSConstants4.h", "VSHelper4.h", "VSScript4.h"]:
        shutil.copyfile(include_dir / header, nested / header)
    ...
    (pc_dir / "vapoursynth.pc").write_text(
        "\n".join(
            [
                ...
                "",
                "Name: vapoursynth",
                "Description: VapourSynth R77 wheel headers for Retinex CI builds",
...
def prepare_vapoursynth(deps: Path) -> Path:
    ...
    write_include_shim(out / "vapoursynth")
