...
def _default_prebuilt_url(version: str) -> str:
    ...
    tag = os.environ.get("NNEDI3CL_PREBUILT_TAG") or f"vapoursynth-nnedi3cl-v{version}"
...
def _ensure_supported_platform() -> None:
    if not _supports_prebuilt():
        raise RuntimeError("vapoursynth-nnedi3cl wheels support only Windows and Linux x86_64")
