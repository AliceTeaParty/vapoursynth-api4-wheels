...
def build_stage(root: Path, variant: str = "cu129", *, windows_resource: bool = False) -> Path:
    ...
    for name in ("vsncnn.so", "vsov.so", "vstrt.so"):
        (stage / name).write_bytes(name.encode())
    for name in ("libnvinfer.so.11", "libnvinfer_builder_resource_sm90.so.11", "trtexec", "trtexec-build.json"):
    ...
    if windows_resource:
        (stage / "vsmlrt-cuda" / "libnvinfer_builder_resource_win_sm90.so.11").write_bytes(b"windows")
...
class LinuxPayloadTests(unittest.TestCase):
    ...
    def test_oversized_payload_splits_into_volumes(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            (stage / "vsmlrt-cuda" / "libnvinfer.so.11").write_bytes(os.urandom(200_000))
            output = root / "vs-mlrt-linux-x64-cu129.zip"
            ...
            with _archive.open_payload(volumes) as archive:
                self.assertIn("vsmlrt/vsmlrt-cuda/libnvinfer.so.11", archive.namelist())
