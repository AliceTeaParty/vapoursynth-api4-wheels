...
def _default_prebuilt_url(version: str) -> str:
    ...
    tag = os.environ.get("SMOOTHUV_PREBUILT_TAG") or f"vapoursynth-smoothuv-v{version}"
...
def _ensure_supported_platform() -> None:
    if not _supports_prebuilt():
        raise RuntimeError("vapoursynth-smoothuv wheels support only Windows and Linux x86_64")
