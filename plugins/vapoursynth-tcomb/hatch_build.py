...
def _default_prebuilt_url(version: str) -> str:
    ...
    tag = os.environ.get("TCOMB_PREBUILT_TAG") or f"vapoursynth-tcomb-v{version}"
...
def _ensure_supported_platform() -> None:
    if not _supports_prebuilt():
        raise RuntimeError("vapoursynth-tcomb wheels support only Windows and Linux x86_64")
