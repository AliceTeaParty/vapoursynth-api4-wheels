...
def find_built_plugin(build_dir: Path) -> Path:
    for candidate in [
        build_dir / "libfft3dfilter.dll",
        build_dir / "fft3dfilter.dll",
    ]:
        if candidate.exists():
            return candidate
    raise FileNotFoundError(build_dir / "libfft3dfilter.dll")


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Build and package FFT3DFilter with MSYS2 UCRT64.")
    ...
    dll = find_built_plugin(build_dir)

    shutil.copy2(dll, pkg_dir / "fft3dfilter.dll")
