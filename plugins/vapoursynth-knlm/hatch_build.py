...
def _default_prebuilt_url(version: str) -> str:
    ...
    tag = os.environ.get("KNLMEANSCL_PREBUILT_TAG") or f"vapoursynth-knlm-v{version}"
...
class CustomHook(BuildHookInterface[Any]):
    ...
    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        del version
        if not _supports_prebuilt():
            raise RuntimeError("vapoursynth-knlm wheels support only Windows and Linux x86_64")
