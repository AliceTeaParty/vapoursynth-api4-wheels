...
def _default_prebuilt_url(version: str, asset: str) -> str:
    ...
    tag = os.environ.get("MISC_PREBUILT_TAG") or f"vapoursynth-misc-v{version}"
...
def _build_wheel(wheel_directory: str) -> str:
    if _platform_name() != "win32" and not _platform_name().startswith("linux"):
        raise RuntimeError("vapoursynth-misc supports only Windows and Linux x86_64")
    if not _is_x86_64():
        raise RuntimeError("vapoursynth-misc supports only Windows and Linux x86_64")
